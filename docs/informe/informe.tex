...
escena (barco, isla, faro, skybox, sol, luna, nubes y peces), la simulación de flotación real del
barco sobre la superficie deformada y un evento dinámico de \emph{ola
errática} que perturba la escena periódicamente. Además, se documentan mejoras
de arquitectura y rendimiento: reutilización de modelos \texttt{.obj} en GPU,
control de memoria para evitar cargas redundantes, y lógica de colisiones
entre barcos e isla.
\end{abstract}
...
\end{itemize}
En la versión actual se amplió el rango de \texttt{pitch} para permitir
rotación vertical hacia arriba y hacia abajo (mirada libre en ambos sentidos),
manteniendo límites para evitar singularidades cerca de la vertical extrema.

\section{Elementos adicionales}
Además del océano, la escena incluye elementos estáticos, dinámicos y
ambientales que se integran
visualmente con posición, escala e iluminación coherentes con la superficie
animada.

\subsection{Barco y flotación realista}
El \texttt{Barco} (\texttt{src/Barco.cpp}) utiliza en primer lugar un modelo
\texttt{.obj} texturizado (barco medieval). Como \emph{fallback} conserva una
versión procedural (casco, mástil y vela) cuando el modelo externo no está
disponible. Su posición vertical y orientación se recalculan cada frame
consultando directamente la fórmula del océano en su posición $(x,z)$, sin
ningún motor de física:
...
Además, la navegación del barco sigue una órbita circular:
\begin{align}
x(t) &= x_c + R\cos(\theta(t)) \\
z(t) &= z_c + R\sin(\theta(t)) \\
\theta(t) &= \theta_0 + \omega t
\end{align}
donde $R$ es el radio de órbita y $\omega$ la velocidad angular. La orientación
de proa (\emph{yaw}) persigue suavemente a la tangente de la trayectoria para
evitar saltos bruscos al invertir dirección tras colisiones.
...
\subsection{Sol, luna y ciclo día/noche}
La escena incorpora un ciclo temporal continuo definido en
\texttt{CicloDiaNoche.h}. El ángulo solar se modela como:
\begin{equation}
\theta_{\text{sol}}(t)=2\pi\left(\frac{t \bmod T}{T}\right),\quad T=60\text{ s}
\end{equation}
y la luna se ubica en oposición:
\begin{equation}
\theta_{\text{luna}}(t)=\theta_{\text{sol}}(t)+\pi
\end{equation}
Con este esquema, ambos astros recorren el mismo arco celeste en fases
opuestas. La posición se obtiene con radio fijo $R_{\text{cielo}}$:
\begin{equation}
\mathbf{p}(t)=\left(R_{\text{cielo}}\cos\theta,\ R_{\text{cielo}}\sin\theta,\ 0\right)
\end{equation}

La transición día/noche usa un factor suave (\emph{smoothstep}) basado en la
altura del sol:
\begin{equation}
f_{\text{día}}(t)=s\!\left(\sin(\theta_{\text{sol}}(t))\right),\quad f_{\text{día}}\in[0,1]
\end{equation}
donde $s(\cdot)$ denota la interpolación cúbica suave. Este factor controla la
mezcla del color de luz principal entre un tono diurno y uno nocturno.

\subsection{Nubes}
Las nubes (\texttt{src/Nube.cpp}) se generan proceduralmente como clústeres
de esferas achatadas con semilla aleatoria por instancia. Su deriva horizontal
se define con una función periódica envuelta (reaparición al salir del rango):
\begin{equation}
x'(t)=\mathrm{wrap}\!\left(x_0+vt,\ [-L,\ L]\right)
\end{equation}
Esto evita crecimiento ilimitado de coordenadas y mantiene densidad visual
constante de nubes en pantalla.

\subsection{Peces}
Se agregaron peces animados (\texttt{src/Pez.cpp}) en varios grupos. Cada pez
usa órbita local en el plano $xz$ y nado vertical sinusoidal, siempre por
debajo de la superficie real del agua:
\begin{align}
x(t) &= x_g + r\cos(\theta_0+\omega t), \\
z(t) &= z_g + r\sin(\theta_0+\omega t), \\
y(t) &= h(x(t),z(t),t) - d + 0.15\sin(2t+\phi).
\end{align}
De este modo, los peces heredan naturalmente las deformaciones de olas
normales y erráticas sin lógica especial adicional.
...
\section{Colisiones y dinámica de escena}
Se añadieron colisiones entre entidades móviles y obstáculos:
\begin{itemize}
    \item \textbf{Barco--barco}: detección por distancia horizontal entre
    centros, comparada contra la suma de radios de colisión.
    \item \textbf{Barco--isla}: detección por distancia al centro de la isla
    con radio efectivo combinado.
\end{itemize}

Sea $\mathbf{p}_a=(x_a,z_a)$ y $\mathbf{p}_b=(x_b,z_b)$:
\begin{equation}
d=\|\mathbf{p}_a-\mathbf{p}_b\|_2=\sqrt{(x_a-x_b)^2+(z_a-z_b)^2}
\end{equation}
Hay colisión si $d<r_a+r_b$. Para barco--barco, el impacto se aproxima por:
\begin{equation}
I=|v_a|+|v_b|,\quad v=R\omega
\end{equation}
Si $I$ supera un umbral, ambos barcos pasan a estado de hundimiento; en caso
contrario invierten sentido de giro. Además, se aplica \emph{cooldown} temporal
para evitar múltiples disparos de la misma colisión en frames consecutivos.

\section{Optimización de carga \texttt{.obj} y memoria GPU}
Una mejora importante fue eliminar cargas redundantes de modelos pesados en
cada instancia. Antes, cada barco/pez podía cargar su propia copia de malla y
textura; ahora \texttt{Escena} realiza una única carga por tipo y comparte el
recurso en múltiples instancias (patrón de recurso compartido por puntero no
propietario en los elementos).

Beneficios directos:
\begin{itemize}
    \item Menor consumo de VRAM (sin duplicar VBO/VAO/texturas por instancia).
    \item Menor tiempo de creación de entidades.
    \item Mayor estabilidad al escalar la cantidad de barcos y peces.
\end{itemize}

Adicionalmente, en \texttt{ObjetoRenderizableTexturado} se incorporó un
\texttt{switch} por cantidad de canales de la imagen para seleccionar formato
OpenGL (\texttt{GL\_RED}, \texttt{GL\_RGB}, \texttt{GL\_RGBA}), mejorando
compatibilidad de texturas y evitando suposiciones incorrectas de formato.
...
\begin{itemize}
    ...
    la creación de elementos estáticos base (isla, faro, skybox). En la
    versión actual, los barcos y peces se instancian directamente desde
    \texttt{Escena} para inyectar modelos compartidos ya cargados en GPU.
...
\newpage
Ciclo día/noche (sol y luna):
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.48\textwidth]{images/captura_dia_sol.png}
  \includegraphics[width=0.48\textwidth]{images/captura_noche_luna.png}
  \caption{Comparación del ciclo iluminación diurna/nocturna: sol visible de día y luna visible de noche.}
\end{figure}

Nubes y peces:
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.48\textwidth]{images/captura_nubes.png}
  \includegraphics[width=0.48\textwidth]{images/captura_peces_cardumen.png}
  \caption{Nubes procedurales en deriva y grupos de peces animados bajo la superficie.}
\end{figure}

Colisiones:
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.48\textwidth]{images/captura_colision_barcos.png}
  \includegraphics[width=0.48\textwidth]{images/captura_colision_isla.png}
  \caption{Eventos de colisión barco--barco y barco--isla durante la simulación.}
\end{figure}
...
rendimiento al decidir qué se calcula dónde. La incorporación de sol/luna,
nubes y peces elevó la complejidad visual sin sacrificar estabilidad gracias a
la reutilización de recursos en GPU. Asimismo, la lógica de colisiones y
estados de barco (normal, volcado, hundimiento) integró dinámica emergente
coherente con la física simplificada del sistema.
